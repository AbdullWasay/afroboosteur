...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate unique discount code
    const code = generateDiscountCode(coach.firstName + coach.lastName, discountPercentage);
    ...
    if (!existingCodeSnapshot.empty) {
      ...
      const newCode = generateDiscountCode(coach.firstName + coach.lastName, discountPercentage) + Math.random().toString(36).substr(2, 3);
      return NextResponse.json(
    ...
    // Generate QR code for the discount card
